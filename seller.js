 ...
 */
 ...
 /**
 * get Seller using carid for chat
 */
controller.post('/getChatDetails',async function(req,res){

	let bid  =  await Bid.findOne({car_id:req.body.carId,bid_acceptance:'accepted'});
	
	 console.log('hii--------------------------------',bid)
	let dealer = await Dealer.findOne({_id:bid.dealer_id},{name:1,profile_pic:1,username:1});
	let chat = await Chat.find({seller_id:req.body.seller_id,dealer_id:bid.dealer_id}).sort({created_at:-1}).limit(10);
  
	return res.status(def.API_STATUS.SUCCESS.OK).send({chat:chat,dealer:dealer});
